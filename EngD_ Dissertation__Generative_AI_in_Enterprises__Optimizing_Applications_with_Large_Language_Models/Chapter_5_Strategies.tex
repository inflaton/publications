...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Open-weight local} (7 models): GPT-OSS (20B, 120B) at three reasoning levels, GLM-4.7-Flash, Gemma3:27b, Llama4:Scout, Granite4:32b---deployed on NVIDIA DGX Spark (128\,GB unified memory) via Ollama
    \item \textbf{Open-weight via API} (8 models): DeepSeek-Chat-v3.1, Kimi-K2, GLM-5, Qwen3-235b, MiniMax-M2.5---with and without reasoning/thinking variants
\end{itemize}
...
\textbf{Reasoning effort has opposite effects across families.}  The optimal reasoning level is family-dependent rather than universal:
\begin{itemize}[leftmargin=*]
    \item \textbf{GPT-5:} Low reasoning is optimal---93.14~F1 at \$6.25 versus 92.14~F1 at \$24.50 for high (performance \emph{degrades} with additional reasoning at 4$\times$ the cost).
    \item \textbf{GPT-5.2:} Reasoning effort has minimal impact ($\sim$92.5~F1 across all levels); low is the value choice at \$3.43 versus \$5.74 for high.
    \item \textbf{Open-weight:} High reasoning is critical---GPT-OSS:120b improves from 90.11 to 92.10~F1 (+1.99\,pp) and GPT-OSS:20b from 88.74 to 91.36~F1 (+2.62\,pp) with increasing effort.
    \item \textbf{Claude:} Thinking produces the most dramatic and asymmetric effects---Sonnet~4.5 gains +10.13~F1 (precision improves by +15.22\,pp while maintaining recall), Haiku~4.5 gains +8.55~F1, yet Opus~4.5 loses $-$0.25~F1 at 2.6$\times$ cost.  Without thinking, all Claude models exhibit high recall (92--93\%) but poor precision (70--79\%), defaulting to predicting ``match'' without deeper analysis.
\end{itemize}
...
Only 13 of 46 configurations form the Pareto frontier, dominated by the GPT ecosystem (GPT-OSS variants, GPT-5-mini, GPT-5.2, GPT-5) plus Gemma3:27b and Qwen3-235b.  No Claude or GLM configuration reaches the frontier.

These findings directly inform model selection for production entity matching systems such as GAIME, where the value tier (GPT-OSS at \$0.46--\$1.20) offers near-frontier accuracy with infrastructure costs compatible with high-volume transaction processing~\cite{huang2026entity}.
