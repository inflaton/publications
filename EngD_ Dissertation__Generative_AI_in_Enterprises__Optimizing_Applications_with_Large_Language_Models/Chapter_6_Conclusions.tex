...
Having previewed the key contributions organized by type in Chapter~\ref{ch:introduction} (Section~\ref{sec:contributions}), we now provide a detailed synthesis organized by research pillar, reflecting on what was achieved across the research portfolio spanning seventeen peer-reviewed publications at premier AI/NLP venues.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Agentic Success Rate (ASR):} We introduced a trajectory-level workflow fidelity metric grounded in process-mining conformance checking that compares observed and expected agent execution sequences at the transition level. Across 18 LLMs and 90{,}000 task instances, ASR exposes systematic procedural deviations invisible to TSR and HF1---7 of 8 top models skip confirmation checkpoints during payment checkout despite near-perfect task success---demonstrating that trajectory-level evaluation is essential in regulated domains where compliance and auditability are paramount.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Cross-Domain Validation:} Applications across financial prediction, sentiment analysis, machine translation, logical reasoning, sports predictive analytics, and entity matching validated the generalizability of optimization techniques. F1-tuned models for sports betting achieved 279.48\% returns (Random Forest + GPT-4.1), demonstrating that metric selection must align with downstream objectives.

    \item \textbf{Entity Matching at Scale:} The largest empirical evaluation of LLM-based entity matching to date---46 model configurations across 8 benchmark datasets---revealed that open-weight models rival proprietary APIs (GPT-OSS:120b achieves 92.10~F1, outperforming GPT-4.1 at 20$\times$ lower cost) and that reasoning effort has family-dependent effects: low is optimal for GPT-5, high is critical for open-weight models, and Claude shows extreme asymmetric thinking impacts (+10.13~F1 for Sonnet~4.5, $-$0.25 for Opus~4.5).  Cost-performance Pareto analysis provides actionable budget-tier guidance for production deployment.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
    ...
    \item qwen2.5:32b achieves 100.0\% ASR matching GPT-4.1 on payment workflows, while enabling privacy-preserving edge deployment (Chapter~\ref{ch:agentic})
    \item GPT-OSS:120b achieves 92.10~F1 on entity matching, outperforming GPT-4.1 (90.36) and GPT-4o (89.58) at 20$\times$ lower cost (Chapter~\ref{ch:strategies})
\end{itemize}
...
This task-complexity hypothesis provides principled deployment guidance: reserve reasoning for complex multi-class problems where accuracy gains justify 2.1$\times$--54$\times$ computational overhead, and leverage base models for simpler classification tasks.  Moreover, entity matching evaluation across 46 configurations reveals that reasoning effectiveness is also \emph{family-dependent}: low reasoning is optimal for GPT-5 (93.14 vs.\ 92.14~F1 for high), whereas high reasoning is critical for open-weight models (GPT-OSS:120b gains +1.99~F1), and Claude models exhibit extreme asymmetric sensitivity (+10.13~F1 for Sonnet~4.5, $-$0.25 for Opus~4.5).
...
ASR & Trajectory-level workflow fidelity & \ref{ch:agentic} \\
...
Key architectural principles validated across seven multi-agent systems:
\begin{itemize}[leftmargin=*]
...
The structured error taxonomy and ASR metric enable systematic diagnosis and improvement of multi-agent system reliability.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Learning-based coordination protocols optimized via the structured error taxonomy
...
The research establishes that open-source models, when properly optimized, can achieve performance competitive with proprietary alternatives while offering advantages in customization, privacy, and cost. Across seventeen peer-reviewed publications at premier venues, we demonstrated this principle in RAG systems (Orca-2-13b matching GPT-4), logical reasoning (Qwen2.5-72B surpassing GPT-4o), multi-agent automation (qwen2.5:32b outperforming cloud APIs), entity matching (GPT-OSS:120b outperforming GPT-4.1 at 20$\times$ lower cost), and payment workflow fidelity (qwen2.5:32b achieving 100.0\% ASR matching GPT-4.1).
...
Multi-agent architectures represent a promising paradigm for automating complex workflows that traditionally require human coordination. Our invoice reconciliation system demonstrates that edge-deployed open-source models can outperform cloud APIs while maintaining privacy and reducing operational costs. The structured error taxonomy and ASR metric provide systematic tools for diagnosing and improving multi-agent reliability.
